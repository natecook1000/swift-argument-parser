...
extension ArgumentDefinition {
  /// - returns: `true` if `self` is an option and can be tab-completed multiple
  ///   times in one command line. For example, `ssh` allows the `-L` option to
  ///   be given multiple times, to establish multiple port forwardings.
