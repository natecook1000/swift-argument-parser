...
public struct CommandConfiguration: Sendable {
  ...
  /// for a custom `EnumerableOptionValue` type), or can describe a static block
  /// of text that extends the description of the argument.
  ...
  ///     non-empty string, the argument parser prints it if the user provides a
  ///     `--version` flag.
  ...
  ///   - defaultSubcommand: The default command type to run if no subcommand is
  ///     given.
  ...
  ///     `nil`, the names are inherited from the parent command, if any, or are
  ///     `-h` and `--help`.
  ...
  ///     derived name if `commandName` is not provided, or `commandName` itself
  ///     if provided.
  public init(
